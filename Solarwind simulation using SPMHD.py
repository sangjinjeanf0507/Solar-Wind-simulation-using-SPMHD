...
dt = 7e-6# 시간 간격 (시뮬레이션 시간 단위, 현재 코드 맥락상 '초'로 가정 가능)

# 헥사곤 격자 간격 (고정된 값, 필요에 따라 조정 가능, 시뮬레이션 길이 단위)
hex_spacing = 0.05
...
center_mass = 10000 # 중심 질량 (비활성화)

# 영구 자석 (자기 쌍극자) 관련 상수 (배경 자기장)
magnet_center = ti.Vector([0.0, 0.0])
initial_magnet_moment_strength = 0.000045 # 자기 쌍극자 모멘트 (SI 단위: Am^2 로 추정, 시뮬레이션 스케일에 맞춰 조정된 값)
...
base_B_magnitude = 0.005 # 시뮬레이션 스케일에 맞춰 조정된 값
...
# --- 밀도 상한선 --- (실제 물리 단위: m^-3)
max_density_cap = 5.0 * 1e6 # 5e6 m^-3 (이는 5 cm^-3를 m^-3으로 변환한 값)
...
special_particle_spawn_x_min = domain_min + 0.1 - 3.0 # 왼쪽 끝에서 약간 오른쪽으로
special_particle_spawn_x_max = domain_min + 0.5 - 3.0 # 왼쪽 끝에서 약간 오른쪽으로, 범위 조정
special_particle_spawn_y_min = domain_min
special_particle_spawn_y_max = domain_max

# 이온, 전자 코어, 전자 헤일로의 초기 속도 (미터/초, m/s)
ion_initial_velocity = ti.Vector([1000000.0, 0.0]) # 400 km/s -> 400,000 m/s
electron_core_initial_velocity = ti.Vector([1000000.0, 0.0]) # 400 km/s -> 400,000 m/s
electron_halo_initial_velocity_low = ti.Vector([800000.0, 0.0]) # 200 km/s -> 200,000 m/s
electron_halo_initial_velocity_high = ti.Vector([7000000.0, 0.0]) # 1000 km/s -> 1,000,000 m/s
...
effective_electron_charge_per_particle = electron_charge * 1e15 # 시뮬레이션 스케일에 맞는 유효 전하 (조정 가능, C)

# 비오-사바르 법칙에 의한 자기장 강도를 조절하는 스케일 팩터.
# 이 값을 조정하여 전자 움직임에 의한 자기장의 영향력을 조절할 수 있습니다.
biot_savart_scale_factor = 5e-14 # (시뮬레이션 스케일에 맞춰 조정된 값, 단위는 복합적)
...
def init_particles_kernel(initial_mag_strength: ti.f32, start_row: ti.i32, end_row: ti.i32, initial_min_radius: ti.f32,
    ...
    # --- 일반 입자 초기화 (헥사곤 격자) ---
    hex_rows_max = 205 # 변경됨: 도메인 크기 증가에 따라 격자 행 수 증가
            ...
            if num_actual_particles[None] < num_particles_max:
                ...
                if ti.random(ti.f32) < probability_of_acceptance:
                    ...
                    if idx < num_particles_max: # 여기서 다시 한번 최종 확인
                        ...
                        vel[idx] = B_initial_direction * 1000.0 # 속도 (m/s)

                        pos[idx] = p_pos_candidate
                        mass[idx] = 1.0 / num_particles_max
                        u[idx] = 1.0
                        rho[idx] = 0.00000000000000000001# 일반 입자의 밀도 (m^-3)
                        acc[idx] = ti.Vector([0.0, 0.0])
                        P_pressure[idx] = (gamma - 1.0) * rho[idx] * u[idx]
                        h_smooth[idx] = 0.04
                        etha_a_dt_field[idx], ae_k_field[idx], etha_a_field[idx] = 0.0, 0.0, u[idx] * mass[idx]
...
def main_simulation_loop():
    ...
    while window.running:
        ...
        if len(electron_core_particles_pos) > 0:
            ...
            canvas.circles(temp_electron_core_circles_field, radius=0.05 / display_range, color=(0.0, 0.0, 1.0)) # 전자 코어: 파란색
        ...
        if len(electron_halo_particles_pos) > 0:
            ...
            canvas.circles(temp_electron_halo_circles_field, radius=0.05 / display_range, color=(0.0, 1.0, 1.0)) # 전자 헤일로: 청록색
        ...
        if particle_arrow_segment_count > 0:
            ...
            canvas.lines(temp_particle_magnetic_arrows_field, color=(0.6, 0.6, 1.0), width=0.001) # width 추가
